...
export default function MusterilerPage() {
  ...
  function pdfIndir() {
    const el = document.getElementById("ekstre-pdf")
    if (!el) return

    html2pdf().from(el).save("ekstre.pdf")
  }

  function whatsappGonder() {
    const mesaj = encodeURIComponent(
      `${musteriAdi(aktif)} için güncel ekstre:
...
{ekstreAcik && (
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
    <div className="bg-[#0B1120] p-6 rounded-2xl w-full max-w-lg">

      <div id="ekstre-pdf">

        <h2 className="text-xl font-bold mb-4">Müşteri Ekstresi</h2>

        <p><b>{musteriAdi(aktif)}</b></p>

        <div className="mt-4 space-y-2">
          <p>Ciro: {tl(a.ciro)}</p>
          <p>Tahsilat: {tl(a.tahsilat)}</p>
          <p>Bakiye: {tl(a.bakiye)}</p>
        </div>

      </div>

      <div className="mt-6 grid grid-cols-2 gap-3">
        <button onClick={pdfIndir} className="bg-blue-600 py-3 rounded-xl font-bold">
          PDF indir
        </button>

        <button onClick={whatsappGonder} className="bg-emerald-600 py-3 rounded-xl font-bold">
          WhatsApp gönder
        </button>
      </div>

      <button
        onClick={() => setEkstreAcik(false)}
        className="mt-4 w-full border border-white/10 py-3 rounded-xl"
      >
        kapat
      </button>
  ...
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
    ...
    <div className="relative h-screen flex bg-[#030712] text-white overflow-hidden w-full max-w-full overflow-x-hidden">
      ...
      <div className={`w-full md:w-[25%] border-r border-slate-800 flex-col ${mobileView === "list" ? "flex" : "hidden md:flex"}`}>
        <div className="border-b border-slate-800 px-5 pb-5 pt-[104px] md:p-5">
...
{ekstreAcik && (
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
    <div className="bg-[#0B1120] p-6 rounded-2xl w-full max-w-lg">
      ...
      <main className={`w-full md:w-[50%] px-4 pb-10 pt-[104px] md:p-6 overflow-y-auto md:overflow-hidden flex-col gap-5 ${mobileView === "detail" ? "flex" : "hidden md:flex"}`}>
        <div className="flex items-start justify-between gap-3">
          <button
            onClick={() => { setMobileView('list'); setActionsOpen(false) }}
            className="md:hidden shrink-0 rounded-xl border border-slate-700 px-4 py-3 text-sm font-semibold text-white"
          >
            ← Liste
          </button>
    ...
    >
      ...
      <button
        ...
        className="md:hidden fixed right-4 top-[calc(env(safe-area-inset-top)+92px)] z-[120] rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold shadow-xl"
      >
        Aksiyon
      </button>
