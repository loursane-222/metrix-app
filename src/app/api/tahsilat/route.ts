...
export async function POST(req: NextRequest) {
  ...
  if (!atolye) return NextResponse.json({ hata: "Atolye bulunamadi." }, { status: 404 });
  const body = await req.json();
  const musteri = await prisma.musteri.findFirst({ where: { id: body.musteriId, atolyeId: atolye.id } });
  if (!musteri) return NextResponse.json({ hata: "Musteri bulunamadi." }, { status: 404 });
  if (body.isId) {
    const is = await prisma.is.findFirst({ where: { id: body.isId, musteriId: musteri.id, atolyeId: atolye.id } });
    if (!is) return NextResponse.json({ hata: "Is bulunamadi." }, { status: 404 });
  }
  const tahsilat = await prisma.tahsilat.create({
    data: { musteriId: musteri.id, isId: body.isId || null, tarih: body.tarih ? new Date(body.tarih) : new Date(), tutar: Number(body.tutar || 0) },
  });
  const musteriAdi = musteri.firmaAdi || (musteri.ad + " " + musteri.soyad).trim() || "Musteri";
  await logActivity({ atolyeId: atolye.id, type: "tahsilat_yapildi", message: musteriAdi + " – " + Number(body.tutar || 0).toLocaleString("tr-TR") + " TL tahsilat yapildi.", refId: tahsilat.id, userId: kullanici.id });
...
export async function GET(req: NextRequest) {
  ...
  const tahsilatlar = await prisma.tahsilat.findMany({
    where: { musteri: { atolyeId: atolye.id }, ...(musteriId ? { musteriId } : {}), ...(isId ? { isId } : {}) },
...
export async function DELETE(req: NextRequest) {
  ...
  const tahsilat = await prisma.tahsilat.findFirst({ where: { id, musteri: { atolyeId: atolye.id } } });
