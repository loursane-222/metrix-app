...
export async function GET(req: NextRequest) {
  ...
  const musteri = await prisma.musteri.findFirst({ where: { id: musteriId, atolyeId: atolye.id } })
...
export async function POST(req: NextRequest) {
  ...
  if (isId) {
    const is = await prisma.is.findFirst({ where: { id: isId, musteriId, atolyeId: atolye.id } })
...
export async function DELETE(req: NextRequest) {
  ...
  if (!tahsilat || tahsilat.musteri.atolyeId !== atolye.id) return NextResponse.json({ hata: 'Tahsilat bulunamadı.' }, { status: 404 })
