...
export function PlakaPlanlayiciV2({
  ...
  function movePiece(e: React.PointerEvent<HTMLDivElement>, result: TipResult, slabIndex: number) {
    ...
    const visualX = clientX - board.left;
    const visualY = clientY - board.top;

    const originalWidth = result.plakaGenislik * scale;
    const originalHeight = result.plakaYukseklik * scale;

    const ratioX = originalWidth / board.width;
    const ratioY = originalHeight / board.height;

    const logicalX = visualX * ratioX;
    const logicalY = visualY * ratioY;
            ...
            return {
              ...
              yerlesim: slab.yerlesim.map((piece, i) =>
                i === dragging.pieceIndex
                  ? {
                      ...piece,
                      x: Math.max(0, Math.round(logicalX / scale - piece.genislik / 2)),
                      y: Math.max(0, Math.round(logicalY / scale - piece.yukseklik / 2)),
                    }
            };
          }
        };
    ...
    <div className="space-y-6">
      ...
      <style jsx global>{`
        @media (max-width: 767px) {
          .mobile-slab-board {
            --mobile-slab-width: min(calc(100vw - 112px), calc(var(--slab-w) * var(--slab-scale) * 1px));
            --mobile-slab-ratio: calc(var(--mobile-slab-width) / (var(--slab-w) * var(--slab-scale) * 1px));

            width: var(--mobile-slab-width) !important;
            height: calc(var(--mobile-slab-width) * var(--slab-h) / var(--slab-w)) !important;
          ...
          .mobile-slab-board > div {
            transform: scale(var(--mobile-slab-ratio)) !important;
              ...
              {result.slabs.map((slab) => (
                <div key={slab.index} className="overflow-x-auto rounded-3xl bg-slate-50 p-4">
                  ...
                  <div
                    ...
                    style={{ touchAction: "none" }} className="mobile-slab-board relative overflow-hidden border-[3px] border-slate-950 bg-cover bg-center shadow-inner"
                    style={{
                      "--slab-w": result.plakaGenislik,
                    "--slab-h": result.plakaYukseklik,
                    "--slab-scale": scale,
                  ...
                  >
                    {slab.yerlesim.map((p, pieceIndex) => (
                      <div
                        ...
                        className="absolute flex cursor-grab touch-none flex-col items-center justify-center border-2 border-rose-600 bg-rose-500/25 p-1 text-center text-xs font-black text-slate-950 backdrop-blur-[1px]"
                        style={{
                          left: p.x * scale,
                          top: p.y * scale,
